...
// mem access count
bool X86CacheAnalysis::memAccessInstruction(const MachineInstr *MI) {
   unsigned op_code = MI->getOpcode();
   const MachineBasicBlock *MB = MI->getParent();
   MachineBasicBlock::const_iterator MBBI(MI);
   int inst_num = std::distance(MB->begin(), MBBI);
...
int X86CacheAnalysis::memAccessCountBasicBlock(const MachineBasicBlock *MB) {
    if (tsgx_debug) std::cout << "[memAcessCountBasicBlock] block num: " << MB->getNumber() << "\n";
 
    int mem_access_count = 0; 
    for (MachineBasicBlock::const_iterator I = MB->begin(), IE = MB->end(); I != IE; ++I) {
	const MachineInstr &MI = *I;
    //for (auto &I:*MB) {  
	if (memAccessInstruction(&MI)) mem_access_count++;
    }
    return mem_access_count;
}
